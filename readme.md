...
* [Uso Local (Sin Docker)](#uso-local-sin-docker)
    ...
    * [Ejecutar el Scraper con Modelo ML (`scraper_model_ia.py`)](#ejecutar-el-scraper-con-modelo-ml-scraper_model_iapy)
...
* Google Chrome y ChromeDriver (para `script.py` y `scraper_model_ia.py` si se ejecutan localmente y no están ya en el PATH o especificados en el Dockerfile)
...
* **`[settings]`**:
    * `output_csv_filename`: Nombre del archivo CSV para guardar los datos procesados localmente.
...
2.  **Ejecutar el contenedor Docker:**
    ...
    * Para ejecutar `scraper_model_ia.py` dentro de Docker, puedes:
        ...
        * O, si el `Dockerfile` está configurado para aceptar comandos, pasar el comando al `docker run`:
            ...
            docker run --rm yogonet-scraper python model_ML/scraper_model_ia.py
            ```
...
El script `main.py` es el punto de entrada principal para el proceso de scraping tradicional (usando `modules/scraper.py`), procesamiento y carga a BigQuery.

1.  **Asegúrate de tener las dependencias instaladas:**
    ```bash
    pip install -r requirements.txt
    ```
2.  **Configura `config/config.ini`** como se describió anteriormente, especialmente las credenciales y configuración de BigQuery si deseas usar esa funcionalidad.
3.  **Ejecuta el script:**
    ```bash
    python main.py
    ```
    El script realizará las siguientes acciones:
    * Cargará la configuración desde `config/config.ini`.
    * Ejecutará el scraper (`modules/scraper.py`).
    * Procesará los datos extraídos (`modules/processor.py`).
    * Guardará los resultados en un archivo CSV en la carpeta `output/` (el nombre del archivo se toma de `config.ini`, ej: `output/yogonet_news_data.csv`).
    * Intentará cargar los datos procesados a BigQuery si la configuración en `config.ini` está completa y no son los valores placeholder (`modules/bigquery_handler.py`).

### Ejecutar el Scraper con Modelo ML (`scraper_model_ia.py`)

El script `model_ML/scraper_model_ia.py` utiliza un modelo de Machine Learning pre-entrenado (`model_ML/extractor_model.pkl`) para identificar y extraer datos de la página.

1.  **Asegúrate de tener las dependencias instaladas**, incluyendo las específicas para ML (ej. `scikit-learn`, `nltk`, `joblib`). Estas deberían estar incluidas en `requirements.txt`.
...
4.  **Ejecuta el script:**
    ```bash
    python model_ML/scraper_model_ia.py
    ```
    El script realizará las siguientes acciones:
    ...
    * Procesará estas predicciones para extraer el contenido.
    * Mostrará los primeros 5 resultados extraídos en la consola.
...
El script `deploy.sh` está diseñado para automatizar el proceso de construcción de la imagen Docker, subirla a Google Artifact Registry y desplegarla como un Cloud Run Job.
...
2.  **Crea un archivo `config/config_deploy.env`** o asegúrate de que la sección `[gcp_deploy]` en `config.ini` esté completa y que el script `deploy.sh` esté adaptado para leer de la fuente correcta. El script proporcionado espera `config/config_deploy.env`.
...
3.  **Asegúrate de que el script `deploy.sh` tenga permisos de ejecución:**
    ```bash
    chmod +x deploy.sh
    ```
4.  **Ejecuta el script desde la raíz del proyecto:**
    ...
    ```
    El script realizará:
    * Configuración de `gcloud` para el proyecto.
    * Habilitación de APIs necesarias.
    * Creación (si no existe) del repositorio en Artifact Registry.
    * Construcción y subida de la imagen Docker usando Cloud Build.
    * Despliegue o actualización del Job en Cloud Run.

### Entrenamiento del Modelo ML

Los scripts para entrenar el modelo de Machine Learning se encuentran en la carpeta `model_ML/training_model/`:

1.  **`collect_html_for_training.py`**:
    ...
    * **Salida**: Los archivos HTML se guardan en `model_ML/training_model/training_data/html_blocks/` con nombres como `block_1.html`, `block_2.html`, etc.
    * **Ejecución**:
        ```bash
...
2.  **`labels.json`**:
    * **Propósito**: Este archivo es crucial y **debe ser creado o actualizado manualmente** después de ejecutar `collect_html_for_training.py`. Contiene las "etiquetas" o la "verdad fundamental" para el entrenamiento. Para cada archivo `block_X.html` guardado, debes inspeccionar su contenido y identificar los XPaths precisos de los elementos que corresponden al título, kicker, y la URL de la imagen.
    ...
    * **Ejemplo de una entrada en `labels.json`**:
        ```json
        {
          "block_1.html": [
...
3.  **`train_model.py`**:
    * **Propósito**: Este script es el corazón del proceso de entrenamiento. Realiza lo siguiente:
        * Carga las etiquetas desde `labels.json`.
        * Lee cada bloque HTML referenciado en `labels.json`.
        * Para cada nodo dentro de los bloques HTML, extrae un conjunto de características (nombre de la etiqueta, número de hijos, clases CSS, longitud del texto, etc.).
        * Asocia estas características con el `role` (etiqueta) correspondiente del archivo `labels.json`.
        * Preprocesa las características (escalado para numéricas, one-hot encoding para categóricas).
        * Entrena un modelo clasificador (RandomForestClassifier en este caso) para predecir el `role` de un nodo HTML basado en sus características.
    * **Salida**: El pipeline completo del modelo entrenado (que incluye el preprocesador y el clasificador) se guarda como `model_ML/extractor_model.pkl`. Este archivo es el que luego utiliza `scraper_model_ia.py`.
    * **Ejecución**:
        ```bash
